...
earn_exact <- read.columns("wfw90.dat", 203:208)
earn2 <- read.columns("wfw90.dat", 209:210)
...
tense <- read.columns("wfw90.dat", 128)
angry <- read.columns("wfw90.dat", 134)
yearbn <- read.columns("wfw90.dat", 196:197)
...
mother_education <- ifelse(momed>18, NA, momed)
father_education <- ifelse(fathed>18, NA, fathed)
...
write.csv(earnings_clean, "earnings_v2.csv", row.names=FALSE)
