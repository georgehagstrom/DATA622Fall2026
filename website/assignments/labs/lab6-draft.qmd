...
Business, which collaborated with a local supermarket chain called Dominick's Finer Foods, to study the impact of prices, advertisting, and
...
and sales for different brands at different Dominick's stores. There is a dataset description here: [oj_dictionary.qmd]().
...
$$
\log(\mathrm{SALES})  = \epsilon\log(\mathrm{PRICE}) 
$$
...
fitting to the original dataset. Use the same selection of confounders as in the original vignette (The 'W' matrix), excluding 'week', 'store', 'price', 'INCOME', and 'logmove', and make sure to create dummies/OneHot and apply the 'StandardScaler'. Then put the modifier variable 'INCOME' variable into a matrix called "X" and standardize it. Then you will simulate a relationship between your confounders and the price, you can use code like
this: 'T_sim = 0.8 + something random based on covariates and modifier', with the 0.8 there to get you the same scale of price variables. Look to the simulation code earlier in the 'EconML' vignette for inspiration.
...
between 'Y_sim', 'T_sim', and 'X' nonlinear using something like this: 'Y_sim = (-2.5 \* np.tanh(2.0\*X))\*T_sim + something random'.
...
Also plot the predicted elasticity and the true elasticity for each simulated observation. 
Comment on the performance of both models on the simulated data. 
...
(b) Compute the R-score on the testing/validation set to determine the strength of the heterogeneity. What is your interpretation of the R-score value?
...
(whethere the item was advertised that week), and the range should be up to 'c_t=0.1' and 'c_y=0.3'. Compute the sensitivity check for the
...
discount brand, 'minute.maid' as the mid-range brand, and 'tropicana' as the premium brand. Add these three brands (this will be two additional variables) to the modifier matrix X and refit the model. Calculate the feature importances for all the modifiers and plot the
elasticity as a function of income for each of the three brands. How do the elasticities differ by brand?

(b) Compute the R-score for the brand-modifier model. How does it compare to the R-score for the model without the brands that you calculated in 3(b)?
